...
# Prefer GHCR_TOKEN already in the environment (Actions). Else read .env.secrets
# without `source` so bcrypt `$` in other keys is not expanded.
if [[ -z "${GHCR_TOKEN:-}" && -f .env.secrets ]]; then
